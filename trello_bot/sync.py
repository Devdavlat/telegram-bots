...
def sync_board_users(board_member_ids, board_id):
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        for member_id in board_member_ids:
            cur.execute(queries.GET_BOARD_BY_TRELLO_ID, member_id, )
            user = cur.fetchone()
            if user:
...
def sync_lists(trello, trello_board_id, board_id):
    boards_lists = trello.get_lists_on_a_board(trello_board_id)
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        for board_list in boards_lists:
            trello_list_id = board_list.get('id')
            cur.execute(
                queries.UPSERT_LIST,
                (board_list.get('name'), (board_list.get('id')), board_id)
            )
            conn.commit()
            cur.execute(queries.GET_LISTS_BY_TRELLO_ID, trello_list_id)
            db_list = cur.fetchone()
...
def sync_card_members(trello_card_id, user_ids):
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        ...
        if not data:
            ...
            must_remove_ids = list(set(existing_user_ids).difference(set(user_ids)))
